...
def receiveOnePing(mySocket, ID, timeout, destAddr):
    ...
    while 1:
        ...
        if type != 0 or code != 0 or ID != id:
            return "Not the correct type or code or ID"
        else:
            ...
            nob = len(recPacket)
            # number of bytes
            return '{} bytes from {} time={:.3f} ms'.format(nob, destAddr, rtt)
...
def ping(host, timeout=1):
    ...
    try:
        ...
        # press ctrl c
        if rtt_cnt != 0:
            print(f"^C--- {dest} ping statistics ---")
            print('round-trip min/avg/max {:.3f}/{:.3f}/{:.3f} ms'.format(rtt_min, rtt_sum / rtt_cnt, rtt_max))

            # python ping.py 127.0.0.1
            # python ping.py 8.8.8.8
            # python ping.py google.com
            # python ping.py cs.stonybrook.edu
            # python ping.py yahoo.co.jp
            # python ping.py sweden.se
            # python ping.py english.customs.gov.cn
